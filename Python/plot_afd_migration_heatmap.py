...
def merge_sim_dict():
    ...
    file_end_all = ['_9', '_8', '_7', '_6', '_5', '_4_5', '_3_4', '_1_3']

    dict_merged = {}

    for file_end in file_end_all:

        path_ = "%s/data/simulation_migration_all_afd%s.pickle" % (utils.directory, file_end)

        with open(path_, 'rb') as handle:
            dict_ = pickle.load(handle)

        key_all = list(dict_.keys())
        for key in key_all:

            dict_merged[key] = dict_[key]

    with open(simulation_migration_all_afd_path, 'wb') as handle:
        pickle.dump(dict_merged, handle, protocol=pickle.HIGHEST_PROTOCOL)


        
def make_plot():

    per_asv_ks_dict = plot_afd_migration.make_per_asv_ks_dict()
    ...
    # plot simulation
    simulation_dict = slm_simulation_utils.load_simulation_migration_all_afd_dict()
    ...
    for treatment_idx, treatment in enumerate(['no_migration', 'parent_migration', 'global_migration' ]):
        ...
        observed =  per_asv_ks_dict[(treatment.capitalize(), 4)]['mean_over_asv_ks_rescaled_12_vs_18']
        ...
        for tau in tau_all:
            ...
            for sigma in sigma_all:
                ...
                ks_rescaled_12_vs_18_i = np.asarray(simulation_dict[tau][sigma]['mean_over_asv_ks_rescaled_12_vs_18'][treatment])

                if len(ks_rescaled_12_vs_18_i) == 0:
                    mean_ks_rescaled_12_vs_18 = np.nan
                    mean_error_ks_rescaled_12_vs_18 = np.nan
        ...
        ks_rescaled_12_vs_18_all_flat = ks_rescaled_12_vs_18_all.flatten()
        ks_rescaled_12_vs_18_all_flat = ks_rescaled_12_vs_18_all_flat[(~np.isnan(ks_rescaled_12_vs_18_all_flat)) & np.isfinite(ks_rescaled_12_vs_18_all_flat)]
        ...
        delta_range = max([observed  - np.min(ks_rescaled_12_vs_18_all_flat),  np.max(ks_rescaled_12_vs_18_all_flat) - observed])
        pcm_slope = ax_ks.pcolor(x_axis_log10, y_axis, ks_rescaled_12_vs_18_all, cmap=cmap, norm=colors.TwoSlopeNorm(vmin=observed-delta_range, vcenter=observed, vmax=observed+delta_range))

        pcm_slope.cmap.set_under('w')
        pcm_slope.cmap.set_over('w')
        pcm_slope.cmap.set_bad(color='w')
        
        clb_slope = plt.colorbar(pcm_slope, ax=ax_ks)
        #clb_slope.ax.set_ylim(observed-delta_range, observed+delta_range)

        


        clb_slope.set_label(label='Mean KS distance between transfers 12 and 18' , fontsize=9)
        ...
        if treatment_idx == 0:
            original_ticks.remove(1.2)

        elif treatment_idx == 1:
            original_ticks.remove(1.5)

        else:
            original_ticks.remove(0.8)
        ...
        # slope error
        ks_rescaled_12_vs_18_error_all_flat = ks_rescaled_12_vs_18_error_all.flatten()
        ks_rescaled_12_vs_18_error_all_flat = ks_rescaled_12_vs_18_error_all_flat[(~np.isnan(ks_rescaled_12_vs_18_error_all_flat)) & np.isfinite(ks_rescaled_12_vs_18_error_all_flat)]

        pcm_slope_error = ax_ks_error.pcolor(x_axis_log10, y_axis, ks_rescaled_12_vs_18_error_all, cmap='YlOrRd', norm=colors.TwoSlopeNorm(vmin=np.min(ks_rescaled_12_vs_18_error_all_flat), vcenter=np.median(ks_rescaled_12_vs_18_error_all_flat), vmax=np.max(ks_rescaled_12_vs_18_error_all_flat)))
...
if __name__=='__main__':
    ...
    print("Making AFD heatmap....")

    #merge_sim_dict()

    make_plot()

    # doesn't include attractor...possibility for revision.
