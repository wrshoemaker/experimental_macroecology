...
for experiment in experiments:
    comm_rep_list_12 = utils.get_s_by_s_migration_test_singleton(transfer=12,migration=experiment[0],inocula=experiment[1])[2]
    comm_rep_list_18 = utils.get_s_by_s_migration_test_singleton(transfer=18,migration=experiment[0],inocula=experiment[1])[2]
    comm_rep_intersect = np.intersect1d(comm_rep_list_12, comm_rep_list_18)
    #comm_rep_intersect.astype(int)

    intersection_dict[experiment] = comm_rep_intersect
...
for experiment in experiments:
    ...
    for transfer in transfers:
        ...
        afd_log10_all, rescaled_afd_log10_all = utils.get_flat_rescaled_afd(rel_s_by_s)

        afd_dict[experiment][transfer] = {}
        afd_dict[experiment][transfer]['afd'] = afd_log10_all
        afd_dict[experiment][transfer]['afd_rescaled'] = rescaled_afd_log10_all

        # intersect
        comm_rep_list = np.asarray(comm_rep_list)
        to_keep_idx = np.asarray([np.where(comm_rep_list==k)[0][0] for k in intersection_dict[experiment] ])
        rel_s_by_s_intersect = rel_s_by_s[:,to_keep_idx]
        afd_log10_intersect_all, rescaled_afd_log10_intersect_all = utils.get_flat_rescaled_afd(rel_s_by_s_intersect)

        afd_dict[experiment][transfer]['afd_intersect'] = afd_log10_intersect_all
        afd_dict[experiment][transfer]['afd_rescaled_intersect'] = rescaled_afd_log10_intersect_all
...
def make_ks_dict():
    ...
    for combo in treatment_combinations:
        ...
        for transfer in transfers:
            ...
            for rescaled_status in rescaled_status_all:
                ...
                ks_statistic, p_value = utils.run_permutation_ks_test(afd_experiment_1, afd_experiment_2, n=n_iter)
    ...
    for experiment_idx, experiment in enumerate(experiments):
        ...
        for rescaled_status in rescaled_status_all:
            ...
            ks_statistic, p_value = utils.run_permutation_ks_test(afd_dict[experiment][transfers[0]][rescaled_status], afd_dict[experiment][transfers[1]][rescaled_status], n=n_iter)

            print(experiment, rescaled_status, ks_statistic, p_value)
