...
for migration_innoculum in migration_innocula:
    ...
    for attractor_idx, attractor in enumerate(attractor_dict.keys()):
        ...
        for transfer in transfers:
            ...
            print(utils.predict_occupancy(s_by_s_attractor))
...
def plot_mean_rel_error():
    ...
    fig.savefig(utils.directory + "/figs/gamma_error.png", format='png', bbox_inches = "tight", pad_inches = 0.5, dpi = 600)
    plt.close()
...
def plot_error_survival():
    ...
    for transfer_idx, transfer in enumerate([12, 18]):

        ax = plt.subplot2grid((1, 2), (0, transfer_idx), colspan=1)
        ...
        for migration_innoculum in utils.migration_innocula:
            ...
            relative_errors = np.absolute(occupancies - predicted_occupancies) / occupancies
            relative_errors = relative_errors[relative_errors>0]
            #relative_errors_log10 = np.log10(relative_errors)
            colors_experiment_transfer = utils.color_dict_range[migration_innoculum][transfer-1]
            #ax.hist(relative_errors, lw=3, alpha=0.8, bins= 15, color=colors_experiment_transfer, histtype='step', label='Transfer %d'%transfer,  density=True)
            ...
            ax.plot(prevalence_range, survival_array, ls='-', lw=3, c=colors_experiment_transfer, alpha=0.8, zorder=2, label=utils.titles_dict[migration_innoculum])


        ax.set_title('Transfer %d' % (transfer), fontsize=12)
        ...
        ax.set_xlabel('Relative error, ' + r'$\epsilon$', fontsize=12)
        ax.set_ylabel('Fraction of ASVs ' + r'$\geq \epsilon$', fontsize=12)


    fig.subplots_adjust(wspace=0.3, hspace=0.3)
    fig.savefig(utils.directory + "/figs/relative_error_survival.png", format='png', bbox_inches = "tight", pad_inches = 0.5, dpi = 600)
    plt.close()




def plot_error_attractor_survival():

    fig = plt.figure(figsize = (8, 8)) #
    fig.subplots_adjust(bottom= 0.15)

    idx_all = [(0,0), (0,1), (1,0), (1,1)]
    ...
    for migration_innoculum_idx, migration_innoculum in enumerate(migration_innocula):
        ...
        for transfer_idx, transfer in enumerate(transfers):
            ...
            attractors = list(attractor_dict.keys())
            attractors.append('All')

            ax = plt.subplot2grid((2, 2), idx_all[count_idx])
            ...
            for attractor in attractors:
                ...
                survival_array = [sum(relative_errors>=i)/len(relative_errors) for i in prevalence_range]
                survival_array = np.asarray(survival_array)
                ax.plot(prevalence_range, survival_array, ls='-', lw=3, c=color, alpha=0.8, zorder=2, label=attractor)
    ...
    fig.savefig(utils.directory + "/figs/relative_error_attractor_survival.png", format='png', bbox_inches = "tight", pad_inches = 0.5, dpi = 600)
    plt.close()




def mean_rel_abundance_attractors():
    ...
    for migration_innoculum in migration_innocula:
        ...
        for transfer in transfers:
            ...
            mean_rel_abundance_p_all = []
            mean_rel_abundance_a_all = []
            ...
            for s_idx, s in enumerate(species):
                ...
                if (s in species_pseudomonadaceae) and (s in species_alcaligenaceae):
                    ...
                    if (s_pseudo_rel_abund>0) and (s_alcal_rel_abund>0):
                        mean_rel_abundance_p_all.append(s_pseudo_rel_abund)
                        mean_rel_abundance_a_all.append(s_alcal_rel_abund)

                elif (s not in species_pseudomonadaceae) and (s in species_alcaligenaceae):
                    n_only_a += 1

                elif (s in species_pseudomonadaceae) and (s not in species_alcaligenaceae):
                    n_only_p += 1

            #print(n_only_a/len(species), n_only_p/len(species))


            ax = plt.subplot2grid((2, 2), idx_all[count_idx])

            ax.scatter(mean_rel_abundance_p_all, mean_rel_abundance_a_all, alpha=0.5, s=15, zorder=2, c='k')

            max_plot = max( [max(mean_rel_abundance_p_all), max(mean_rel_abundance_a_all)] )
            min_plot = min( [min(mean_rel_abundance_p_all), min(mean_rel_abundance_a_all)] )

            ax.plot([min_plot, max_plot],[min_plot, max_plot], lw=2,ls='--',c='k',zorder=1)
            ax.set_xscale('log', basex=10)
            ax.set_yscale('log', basey=10)

            ax.set_xlabel('Mean relative abundance, Pseudomonadaceae', fontsize=10)
            ax.set_ylabel('Mean relative abundance, Alcaligenaceae', fontsize=10)
    ...
    fig.subplots_adjust(wspace=0.3, hspace=0.4)
    fig.savefig(utils.directory + "/figs/mean_rel_abundance_attractors.png", format='png', bbox_inches = "tight", pad_inches = 0.5, dpi = 600)
    plt.close()
