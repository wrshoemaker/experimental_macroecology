...
#s_by_s_1, species, comm_rep_list = utils.get_s_by_s_migration_test_singleton(transfer=12, migration=experiments[0][0], inocula=experiments[0][1])
#s_by_s_2, species, comm_rep_list = utils.get_s_by_s_migration_test_singleton(transfer=18, migration=experiments[0][0], inocula=experiments[0][1])
#n_reads_1 = np.sum(s_by_s_1, axis=0)
#n_reads_2 = np.sum(s_by_s_2, axis=0)

#print(n_reads_1)
#print(n_reads_2)
#rescaled_1 = utils.get_flat_rescaled_afd(s_by_s_1, min_occupancy=0.01)[1]
#rescaled_2 = utils.get_flat_rescaled_afd(s_by_s_2, min_occupancy=0.01)[1]
#ks_statistic, p_value = utils.run_permutation_ks_test(rescaled_1, rescaled_2, n=1)
#print(ks_statistic)
...
def run_best_parameter_simulations():
    ...
    for treatment_idx, treatment in enumerate(treatments_no_innoculum):
        ...
        #print(min_parameter_idx)

        tau_best = tau_all[min_parameter_idx]
        sigma_best = sigma_all[min_parameter_idx]

        print(tau_best, sigma_best, min(euc_dist))

        label = '%s_afd' % treatment
        #print(treatment, tau_best, sigma_best, min(euc_dist))
        slm_simulation_utils.run_simulation_all_migration_fixed_parameters(tau_best, sigma_best, label, n_iter=1000, n_div_to_print=100)
...
def make_plot():
    ...
    for treatment_idx, treatment in enumerate(treatments_no_innoculum):
        ...
        ks_simulated = np.sort(ks_simulated)
