...
def make_plot():
    ...
    for treatment_idx, treatment in enumerate(treatments_no_innoculum):
        ...
        slope_t_test = np.sort(slope_t_test)
        lower_ci = slope_t_test[int(0.025*len(slope_t_test))]
        upper_ci = slope_t_test[int(0.975*len(slope_t_test))]
        ...
        ax.axvline(x=t_slope_all[treatment_idx], ls='--', lw=3, c='k', label='Observed ' +  r'$t_{\mathrm{exponent}}$')
